...
quiz_score_range = (0, 20)  # Range for quiz scores (0 to 20)

# Generating quiz scores
q1_scores = np.random.randint(quiz_score_range[0], quiz_score_range[1], size=num_rows)
q2_scores = np.random.randint(quiz_score_range[0], quiz_score_range[1], size=num_rows)
q3_scores = np.random.randint(quiz_score_range[0], quiz_score_range[1], size=num_rows)
...
# Ensure the score is within 0-20 range and round it to one decimal
final_exam_scores_with_noise = np.clip(np.round(final_exam_scores_with_noise, 1), 0, 20)

# Update the final exam score to be out of 100 and apply error if score < 75
final_exam_scores_out_of_100 = final_exam_scores_with_noise * 5  # Scale score to out of 100

# Apply 10% error if final exam score is below 75
final_exam_scores_updated = []
for score in final_exam_scores_out_of_100:
    if score < 70:
        variation = np.random.uniform(-0.1, 0.1)  # Random variation between -10% and +10%
...
data = {
    ...
    'final_exam': final_exam_scores_updated
}
